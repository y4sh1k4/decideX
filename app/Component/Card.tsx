  ...
  return (
    <div>
      <BackgroundGradient className="rounded-[22px] p-4 sm:p-10 w-[68vw] bg-white dark:bg-zinc-900 flex flex-col items-center gap-3">
        <h2 className="text-2xl font-semibold text-white mb-2">{prompt}</h2>
        <div className="flex justify-evenly">
                    {imgUrl.map((img,index)=>(
                        <button key={index} className={`p-2 border border-3 border-black`}>
                            <Image src={`https://olive-fashionable-mule-815.mypinata.cloud/ipfs/${img}`} width={100} height={30} key={index} alt="vote these images" className="w-[10rem] h-[8rem]"/>
                        </button>
                    ))}
        </div>
        <div className="flex justify-between w-full mt-5">
            ...
            <span className="text-gray-400">Reward bounty: {Number(price)}</span>
        </div>
      </BackgroundGradient>
    </div>
  );
